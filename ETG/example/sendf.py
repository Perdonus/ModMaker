...
SSL_CA_FILE = Path("/root/Heroku/modules/ssl/certificate_ca.crt")
SSL_FULLCHAIN_FILE = Path("/root/Heroku/modules/ssl/fullchain.crt")
...
class FileShareMod(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        current = (self.config.get("base_url") or "").strip()
        normalized = self._normalize_base_url(current)
    ...
    def _normalize_base_url(value: str) -> str:
        base = (value or "").strip()
        if not base:
            return "https://sosiskibot.ru"
        if "91.233.168.135" in base or ":5001" in base:
            return "https://sosiskibot.ru"
        if base.startswith("http://sosiskibot.ru"):
            return "https://sosiskibot.ru"
        return base
    ...
    @staticmethod
    def _get_verify() -> typing.Union[bool, str]:
        if SSL_CA_FILE.exists():
            return str(SSL_CA_FILE)
        if SSL_FULLCHAIN_FILE.exists():
            return str(SSL_FULLCHAIN_FILE)
        return False
