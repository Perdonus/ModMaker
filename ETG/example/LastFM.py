...
class LastFMNowPlayingMod(loader.Module):
    ...
    async def _get_track_info(self) -> Optional[Dict[str, Any]]:
        ...
        track = tracks[0] if isinstance(tracks, list) else None
        if not isinstance(track, dict):
            return None

        now_playing_attr = str(track.get("@attr", {}).get("nowplaying", "")).lower()
        has_date = bool(track.get("date"))
        is_playing = now_playing_attr == "true"

        if has_date or not is_playing:
            return None
