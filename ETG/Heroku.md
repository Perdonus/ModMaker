═══════════════════════════════════════════════════════════════════════════════
ETG UI EXTENSION (ETG Bridge)
═══════════════════════════════════════════════════════════════════════════════

ВАЖНО ДЛЯ UI-МОДУЛЕЙ:
- ВСЕГДА старайся использовать EtgBridge API для UI, интерактивных экранов, рендеров.
- ПОЛУЧАЙ EtgBridge через self.lookup("EtgBridge").
- ЕСЛИ EtgBridge недоступен — покажи понятное сообщение и выйди.
- Для ETG UI используй действия: sheet, dialog, menu, prompt, open_editor, render_html, send_png.
- Учитывай ожидание результата через wait_result (timeout в секундах).
- Поддерживай fallback: если ETG не отвечает — печатай ошибку.
- НЕ придумай новые методы — используй только те, что описаны в EtgBridge.md.

Ниже идет полный стандартный промпт Heroku. Используй его как базу, но
дополнительно учитывай этот ETG UI блок.
